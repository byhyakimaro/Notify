...
function createNotify({color = "#77b901", verified, texto}) {
	this.color = color;
	this.verified = verified;
	this.texto = texto;
}

createNotify.prototype.create = async function() {
	$('.texto').html(this.texto);
	$('svg').css('fill', this.color);
	bg_notificacao.css('background', this.color);
	$('path').attr('d',this.verified ? check : closed);
...
let addNotify = new createNotify({
	verified: true,
	color: "#77b901",
